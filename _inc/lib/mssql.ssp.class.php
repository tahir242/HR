...
class SSP {
	...
	// 		$limit = "TOP ".intval($request['start'])." ".intval($request['length']);
	// 	}
	...
	* Ordering
	*/
	static function order ($table, $request, $columns ) {
		$order = '';
		if ( isset($request['order']) && count($request['order']) ) {
		$orderBy = array();
		$dtColumns = self::pluck( $columns, 'dt' );
		for ( $i=0, $ien=count($request['order']) ; $i<$ien ; $i++ ) {
			...
			'ASC' :
			'DESC';
			$orderBy[] = $table .'.['.$column['db'].'] '.$dir;   // revised for SQL Server
			}
		}
		// print_r($orderBy);
		// die();
	...
	static function orderComplex ($table, $request, $columns ) {
		$order = '';
		if ( isset($request['order']) && count($request['order']) ) {
		$orderBy = array();
		$dtColumns = self::pluck( $columns, 'dt' );
	...
	static function simple ( $request, $sql_details, $table, $primaryKey, $columns ) {
		...
		$order = self::order( $table, $request, $columns );
	  ...
	  // Data set length after filtering  the $where will update info OR will be blank when not doing a search
		$resFilterLength = self::sql_exec( $db, $bindings,
			  ...
			  SET ARITHABORT OFF;
			  SET ARITHIGNORE ON;
			  SET ANSI_WARNINGS OFF; SELECT ".implode(", ", self::pluck($columns, 'db'))." FROM $table $where " );
	...
	*/
	...
	static function complex ( $request, $conn, $table, $primaryKey, $columns, $whereAll, $whereResult=null)
    {
        $bindings = array();
        $db = self::db( $conn );
        ...
        // Build the SQL query string from the request
        $limit = self::limit( $request, $columns );
        $order = self::orderComplex( $table, $request, $columns );
        $where = self::filter( $request, $columns, $bindings );
  ...
  $recordsTotal = $resTotalLength[0][0];
	...
	static function sql_connect ( $sql_details )
	{
		try {
			...
			$db = @new PDO("sqlsrv:Server={$sql_details['host']};Database={$sql_details['db']};Encrypt=true;TrustServerCertificate=true;", $sql_details['user'], $sql_details['pass'], array( PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION ));
		}

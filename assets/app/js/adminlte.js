(function (global, factory) {
  ...
  var SELECTOR_PRELOADER = '#ajaxWait';
...
$(document).delegate(".user-profile", "click", function (e) {
...
let openModal = function ($scope) {
  let $uibModalInstance = $(`<div class="modal fade" role="dialog">
    <div class="modal-dialog modal-${$scope.size}" role="document">
    <div class="modal-content">
    <div class="modal-header">
      <h4 class="modal-title">${$scope.title}</h4>
      <button type="button" data-dismiss="modal" aria-label="Close">
        <span aria-hidden="true">&times;</span>
      </button>
    </div>
        <div class="modal-body" id="modal-body">
          <div class="rawHtml">Loading...</div>
        </div>
      </div>
    </div>
    </div>`);
  ...
  closeModal = function () {
    $uibModalInstance.modal('hide');
  };

  $uibModalInstance.on('hidden.bs.modal', function () {
    $uibModalInstance.remove();
  });

  // Append modal to body and show it
  $('body').append($uibModalInstance);
  $uibModalInstance.modal('show');
};
...
$(".support").on("click", function (e) {
  let $scope = {
      ...
      url: window.baseUrl + "/_inc/value.php?action_type=VIEWSUPPORTMODAL"
  };
  openModal($scope);

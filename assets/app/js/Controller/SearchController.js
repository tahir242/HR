...
docReady(function () {
    ...
    let SearchResponse = (response) => {
        const { valid, content } = response.data;
        if (valid) {
    ...
    $(document).on("click", "#create-submit", function (e) {
        ...
        axios.post(`${window.baseUrl}/_inc/${form.attr("action")}`, formData)
            .then(SearchResponse)
            .catch(handleError);
    ...
    $(document).on("click", "#reset", function (e) {
        ...
        $('#Employee_Name1').val("");
        $('#DOJ1').val("");
        $('#Designation1').val("").trigger("change");
    ...
    $(document).delegate(".view-pdf", "click", function (e) {
        ...
        e.preventDefault();
        $("#ajaxWait").children().show();
        const pdf = $(this).data('file');
        const formData = new FormData();
        formData.append("Scan", pdf);
        formData.append("action_type", "VIEWPDF");
        axios.post(`${window.baseUrl}/_inc/search.php`, formData)
            .then(SearchResponse)
            .catch(handleError);
        $("#ajaxWait").children().hide();
    });
    ...
    // view pdf
    $(document).delegate(".update-button", "click", function (e) {
        e.stopPropagation();
        e.preventDefault();
        $("#ajaxWait").children().show();
        const pdf = $(this).data('file');
        const formData = new FormData();
        formData.append("Scan", pdf);
        formData.append("action_type", "UPDATEDEMOGRAPHIC");
        axios.post(`${window.baseUrl}/_inc/search.php`, formData)
            .then(function (response) {
                const { valid, content } = response.data;
                if (valid) {
    ...
    // search button
    $(document).delegate(".search-button", "click", function (e) {
        e.stopPropagation();
        e.preventDefault();
        $("#rawHtml1").html("");
        $("#search-field").show();
